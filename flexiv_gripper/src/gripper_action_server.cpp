            ...
            if (!robot_->operational()) {
                ...
                while (!robot_->operational()) {
                    if (std::chrono::steady_clock::now() >= deadline) {
                        throw std::runtime_error("Robot did not become operational within "
                            + std::to_string(kOperationalTimeout.count()) + " s. "
                            + flexiv_hardware::DescribeRobotCondition({robot_->connected(),
                                robot_->operational_status(), false}));
                    }
                    RCLCPP_INFO(this->get_logger(), "Waiting for the robot to become operational: %s",
