...
TcpPoseStateBroadcaster::state_interface_configuration() const
{
    ...
    state_interfaces_config.names
        = cartesian_pose_sensor_->get_state_interface_names();
    return state_interfaces_config;
}

controller_interface::return_type TcpPoseStateBroadcaster::init(
    ...
    const rclcpp_lifecycle::State& /*previous_state*/)
{
    ...
    if (sensor_name_.empty()) {
        RCLCPP_ERROR(node_->get_logger(),
            "'sensor_name' parameter has to be specified.");
    ...
    if (frame_id_.empty()) {
        RCLCPP_ERROR(
            node_->get_logger(), "'frame_id' parameter has to be specified.");
        return CallbackReturn::ERROR;
    }

    if (!sensor_name_.empty()) {
        cartesian_pose_sensor_ = std::make_unique<CartesianPoseSensor>(
            CartesianPoseSensor(sensor_name_));
    }

    topic_name_ = node_->get_parameter("topic_name").as_string();
    if (topic_name_.empty()) {
        RCLCPP_ERROR(
            node_->get_logger(), "'topic_name' parameter has to be specified.");
        return CallbackReturn::ERROR;
    }
...
controller_interface::return_type TcpPoseStateBroadcaster::update()
{
    if (realtime_publisher_ && realtime_publisher_->trylock()) {
        ...
        cartesian_pose_sensor_->get_values_as_message(
            realtime_publisher_->msg_.pose);
...
PLUGINLIB_EXPORT_CLASS(flexiv_controllers::TcpPoseStateBroadcaster,
    controller_interface::ControllerInterface)