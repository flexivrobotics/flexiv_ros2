...
- `kinematics_params_file` (default: *empty*) - kinematics YAML file holding this robot's measured parameters, as generated by [`flexiv_calibration`](#robot-calibration). When empty, the nominal parameters shipped in `flexiv_description` are used.
...
Every robot leaves the factory with measured kinematic parameters that differ slightly from the nominal ones shipped in `flexiv_description`. The `flexiv_calibration` package reads the actual parameters from a connected robot and writes them to a kinematics YAML file, which the driver launch files can then load so that the URDF describes your specific robot rather than the model.

Extract the parameters once per robot, and again whenever the robot is re-calibrated or repaired. Reading them requires an RDK professional license:
...
The target file must be outside `flexiv_description`, so that the shipped defaults are left untouched. Pass it to the driver on every subsequent launch:

```bash
ros2 launch flexiv_bringup rizon.launch.py robot_sn:=[robot_sn] robot_type:=Rizon4s kinematics_params_file:=$HOME/flexiv_calib/[robot_sn]_kinematics.yaml
```

For a dual robot setup, run the extraction once per serial number and pass both files:

```bash
ros2 launch flexiv_calibration calibration_correction.launch.py robot_sn:=[robot_sn_left] robot_type:=Rizon4 target_filename:=$HOME/flexiv_calib/left.yaml
ros2 launch flexiv_calibration calibration_correction.launch.py robot_sn:=[robot_sn_right] robot_type:=Rizon4R target_filename:=$HOME/flexiv_calib/right.yaml

ros2 launch flexiv_bringup rizon_dual.launch.py robot_sn_left:=[robot_sn_left] robot_sn_right:=[robot_sn_right] kinematics_params_file_left:=$HOME/flexiv_calib/left.yaml kinematics_params_file_right:=$HOME/flexiv_calib/right.yaml
```

The same arguments are accepted by the corresponding MoveIt launch files, and passing them there matters: `move_group` must build its URDF from the same kinematics file as the driver, or planning and control will disagree.
