...
class PublisherJointPosition(Node):
    def __init__(self):
        super().__init__("publisher_joint_position")
        ...
        publish_topic = "/" + self.controller_name + "/" + "commands"
        ...
        # publisher topic
        if self.controller_name == "forward_position_controller":
            self.publisher_ = self.create_publisher(Float64MultiArray, publish_topic, 1)
        elif self.controller_name == "joint_impedance_controller":
    ...
    def timer_callback(self):
        if self.controller_name == "forward_position_controller":
            ...
            msg = JointPosVel()
            msg.positions = self.goals[self.i]
            self.get_logger().info(f'Publishing: "{msg.positions}"')
...
def main(args=None):
    ...
    publisher_joint_position = PublisherJointPosition()

    rclpy.spin(publisher_joint_position)
    publisher_joint_position.destroy_node()
    rclpy.shutdown()
