...
/**
 * @brief [Non-blocking] Resolve the joints a request addresses.
 * @param[in] requested_names Joint names from the request. Empty addresses every joint in
 * [joint_names], in that order.
 * @param[in] joint_names Every joint the interface covers, in URDF order.
 * @param[out] indices Index into [joint_names] for each addressed joint, in request order.
 * @return False on a name that is not covered or is named twice, with [message] naming it.
 */
...
/**
 * @brief Which of the covered joints a delivery changes, indexed like the joint list. The RDK sets
 * a whole joint group at a time, so this is what lets a request that names one arm leave the other
 * arm's joint group untouched instead of re-sending it.
 */
using JointMask = std::vector<bool>;

/**
 * @brief The three blocking RDK setters, supplied by the hardware interface. Each takes
 * ROS-ordered, full-length vectors plus the mask of entries the request changed, and throws as the
 * RDK does. The hardware interface is where the vectors are split per joint group and the
 * untouched groups are skipped.
 */
struct JointImpedanceSetters
{
    std::function<void(const std::vector<double>& k_q, const std::vector<double>& z_q,
        const JointMask& touched)>
        set_joint_impedance;
    std::function<void(const std::vector<double>& max_torques, const JointMask& touched)>
        set_max_contact_torque;
    std::function<void(const std::vector<double>& inertia_scales, const JointMask& touched)>
...
public:
     ...
     * @param[in] joint_names ROS joint names in URDF order, covering the arm joints the RDK
     * reports impedance bounds for. Every vector the node handles is indexed by this.
...
private:
    ...
    /** @brief Mask marking the addressed joints, indexed like joint_names_. */
    JointMask MaskFor(const std::vector<size_t>& selection) const;

    /** @brief Copy the addressed values into their places in a full-length held vector. */
    static void ApplySelection(std::vector<double>& held, const std::vector<size_t>& selection,
