...
enum class RecoveryPolicy : uint8_t
{
    ...
    /** Recoverable by ClearFault()/ServoOn() without operator intervention. */
    AUTO_RECOVERABLE = 2,
 ...
 * contain hyphens, e.g. Enlight-L-123456, which are not valid characters in a ROS name.
 */
 ...
 * servoed on, operational and in IDLE control mode. Restoring the control mode is deliberately left
 * to a controller restart, which re-initializes the controller's own setpoint and therefore cannot
...
public:
     ...
     */
    explicit RecoveryStateMachine(RobotSystemControl& robot);
