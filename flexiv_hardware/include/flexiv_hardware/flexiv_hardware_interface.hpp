...
private:
    ...
    /**
     * @brief Bring up the recovery and joint impedance nodes on an executor owned here.
     * @return True on success, false if the robot's joint groups cannot be mapped.
     */
    ...
    // Recovery interface, hosted on an executor owned here. ros2_control in Humble hands the
    // hardware component no executor, so the nodes get their own spin thread, which also keeps the
    // blocking system control calls off the real-time control loop.
