...
TEST(OperationalStatus, FaultsAreAutoRecoverable)
{
    ...
    EXPECT_EQ(ClassifyRecoveryPolicy(Connected(OperationalStatus::CRITICAL_FAULT)),
        RecoveryPolicy::AUTO_RECOVERABLE);
    EXPECT_EQ(ClassifyRecoveryPolicy(Connected(OperationalStatus::NOT_SERVO_ON)),
        RecoveryPolicy::AUTO_RECOVERABLE);
}
...
TEST(OperationalStatus, RecoveryStatePointsAtFlexivElements)
{
    const auto message = DescribeRobotCondition(Connected(OperationalStatus::IN_RECOVERY_STATE));
    EXPECT_NE(message.find("Flexiv Elements"), std::string::npos) << message;
}
...
TEST(DriverStateDerivation, EveryOtherNonOperationalConditionIsAFault)
{
    for (const auto operational_status :
        {OperationalStatus::NOT_SERVO_ON, OperationalStatus::MINOR_FAULT,
...
public:
    ...
    OperationalStatus status_ = OperationalStatus::NOT_SERVO_ON;
...
{
    // An E-stop that was just released leaves the robot NOT_SERVO_ON. Stop() switches the control
    ...
    RecoveryStateMachine machine(robot);
    RunToCompletion(machine);

    EXPECT_EQ(robot.stop_calls, 0);
    EXPECT_EQ(robot.servo_on_calls, 1);
...
TEST(RecoverySequence, ReportsTheControlModeTheRobotIsActuallyIn)
{
    ...
    EXPECT_EQ(robot.stop_calls, 0);
    EXPECT_TRUE(machine.succeeded()) << machine.message();
    EXPECT_NE(machine.message().find("RT_JOINT_POSITION"), std::string::npos) << machine.message();
...
TEST(ControlMode, NamesComeFromTheRdkStrings)
{
    ...
    EXPECT_EQ(ControlModeName(flexiv::rdk::Mode::RT_JOINT_POSITION), "RT_JOINT_POSITION");
    EXPECT_EQ(ControlModeName(flexiv::rdk::Mode::RT_JOINT_IMPEDANCE), "RT_JOINT_IMPEDANCE");
...
TEST(RecoverySequence, AFaultThatCannotBeClearedFails)
{
    ...
    EXPECT_EQ(robot.servo_on_calls, 0);
...
TEST(RecoverySequence, RecoveryStateIsRefusedAndTouchesNothing)
{
    // A joint position limit violation needs the recovery operation run from Flexiv Elements, so
    // the sequence must refuse rather than half-try.
    FakeRobot robot;
    ...
    EXPECT_EQ(machine.policy(), RecoveryPolicy::WAIT_OPERATOR);
    EXPECT_FALSE(machine.succeeded());
    EXPECT_EQ(robot.stop_calls, 0);
    EXPECT_EQ(robot.clear_fault_calls, 0);
    EXPECT_EQ(robot.servo_on_calls, 0);
    EXPECT_NE(machine.message().find("Flexiv Elements"), std::string::npos) << machine.message();
}
