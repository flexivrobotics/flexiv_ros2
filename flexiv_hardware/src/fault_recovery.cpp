...
constexpr std::chrono::seconds kServoOnTimeout {5};
...
RecoveryPolicy ClassifyRecoveryPolicy(const RobotCondition& condition)
{
    ...
    switch (condition.operational_status) {
        ...
        case OperationalStatus::NOT_SERVO_ON:
...
std::string DescribeRobotCondition(const RobotCondition& condition)
{
    ...
    switch (condition.operational_status) {
        ...
        case OperationalStatus::NOT_SERVO_ON:
            return "Robot is not servoed on. Recovery will servo it on.";
        ...
        case OperationalStatus::IN_RECOVERY_STATE:
            return "Joint position limit violated. Use Flexiv Elements to run the recovery "
                   "operation, then restart the driver.";
...
{
    const auto& names = flexiv::rdk::OperationalStatusNames();
...
{
    const auto& names = flexiv::rdk::ModeNames();
...
RecoveryStateMachine::RecoveryStateMachine(RobotSystemControl& robot)
: robot_(robot)
...
bool RecoveryStateMachine::Step()
{
    try {
        switch (state_) {
            ...
            case RecoveryState::STOP:
                ...
                TransitionTo(robot_.fault() ? RecoveryState::CLEAR_FAULT : RecoveryState::SERVO_ON);
                break;
            ...
            case RecoveryState::WAIT_FAULT_CLEARED:
                if (!robot_.fault()) {
                    TransitionTo(RecoveryState::SERVO_ON);
            ...
            case RecoveryState::SERVO_ON:
                // ServoOn() throws if the E-stop is not released, so check first and report the
            ...
            case RecoveryState::WAIT_OPERATIONAL:
                if (robot_.operational()) {
                    message_ = "Robot recovered and is operational in "
                               + ControlModeName(robot_.mode()) + " control mode.";
                    TransitionTo(RecoveryState::COMPLETE);
    ...
    if (state_ == RecoveryState::SERVO_ON && DeadlineExceeded(kServoOnTimeout)) {
        Fail("Timed out delivering the servo-on request to the robot.");
    }
